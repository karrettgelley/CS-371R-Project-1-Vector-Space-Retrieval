 ...
 * @author Ray Mooney
 */
public class InvertedIndexProx {
   ...
   */
  public InvertedIndexProx(File dirFile, short docType, boolean stem, boolean feedback) {
   ...
   *                 to index
   */
  public InvertedIndexProx(List<Example> examples) {
    tokenHash = new HashMap<String, TokenInfo>();
    docRefs = new ArrayList<DocumentReference>();
  ...
  protected void indexDocuments() {
    ...
    // Now that all documents have been processed, we can calculate the IDF weights
    // for
  ...
  protected void indexDocument(FileDocument doc, HashMapVector vector) {
    ...
    for (Map.Entry<String, Weight> entry : vector.entrySet()) {
      ...
      // Add an occurrence of this token to the inverted index pointing to this
      // document
  ...
  /**
   * Compute the IDF factor for every token in the index and the length of the
   * document vector for every document referenced in the index.
   */
  protected void computeIDFandDocumentLengths() {
    ...
    while (mapEntries.hasNext()) {
      ...
      // System.out.println(token + " occurs in " + Math.round(numDocRefs) + " docs so
      // IDF=" + idf);
      if (idf == 0.0)
  ...
  public void print() {
    ...
    for (Map.Entry<String, TokenInfo> entry : tokenHash.entrySet()) {
      ...
      for (TokenOccurrence occ : entry.getValue().occList) {
        System.out.println("   " + occ.docRef.file.getName() + " " + occ.count + " times; |D|=" + occ.docRef.length);
      }
    }
  }
  ...
  public Retrieval[] retrieve(HashMapVector vector) {
    ...
    // Iterate through documents in retrievals array
    for (Retrieval r : retrievals) {
      double proxScore = 0.0;
      int numPairs = 0;
      // Convert to file document
      FileDocument f = null;
      switch (docType) {
      case 0:
        f = new TextFileDocument(r.docRef.file, true);
        break;
      case 1:
        f = new HTMLFileDocument(r.docRef.file, true);
        break;
      }

      // Convert to hashMapProxVector
      HashMapProxVector v = f.hashMapProxVector();

      // Iterate through query tokens
      Iterator<Map.Entry<String, Weight>> it = vector.entrySet().iterator();
      while (it.hasNext()) {
        Map.Entry<String, Weight> entry = it.next();
        String token1 = entry.getKey();
        if (it.hasNext()) {
          Map.Entry<String, Weight> entry2 = it.next();
          String token2 = entry2.getKey();
          // Get position list of token1 in document r
          List<Integer> positionsToken1 = v.hashMap.get(token1);
          List<Integer> positionsToken2 = v.hashMap.get(token2);

          if (positionsToken1 != null && positionsToken2 != null) {
            // Calculate proximity between every occurence of query token1 and token2
            for (int i = 0; i < positionsToken1.size(); i++) {
              for (int j = i + 1; j < positionsToken2.size(); j++) {
                numPairs++;
                proxScore += Math.abs(positionsToken1.get(i) - positionsToken2.get(j));
              }
            }
          } else {
            // Penalty for not having token in document
            proxScore += 1000.0;
            numPairs++;
          }
        }
      ...
      proxScore /= numPairs;
      if (Double.isNaN(proxScore)) {
        proxScore = vector.entrySet().size() * 1000.0;
      }
      r.proxScore = proxScore;
    }
  ...
  /**
   * Retrieve the documents indexed by this token in the inverted index, add it to
   * the retrievalHash if needed, and update its running total score.
   *
   * @param token         The token in the query to incorporate.
   * @param count         The count of this token in the query.
   * @param retrievalHash The hash table of retrieved DocumentReferences and
   *                      current scores.
   * @return The square of the weight of this token in the query vector for use in
   *         calculating the length of the query vector.
   */
  public double incorporateToken(String token, double count, Map<DocumentReference, DoubleValue> retrievalHash) {
    TokenInfo tokenInfo = tokenHash.get(token);
  ...
  /**
   * Enter an interactive user-query loop, accepting queries and showing the
  ...
  public void presentRetrievals(HashMapVector queryVector, Retrieval[] retrievals) {
    if (showRetrievals(retrievals)) {
      ...
      if (feedback)
        fdback = new FeedbackProx(queryVector, retrievals, this);
  ...
  /**
   * Print out at most MAX_RETRIEVALS ranked retrievals starting at given starting
   * rank number. Include the rank number and the score.
   */
  public void printRetrievals(Retrieval[] retrievals, int start) {
    ...
    for (int i = start; i < Math.min(retrievals.length, start + MAX_RETRIEVALS); i++) {
      System.out.println(MoreString.padTo((i + 1) + ". ", 4) + MoreString.padTo(retrievals[i].docRef.file.getName(), 20)
          + " Score: " + (MoreMath.roundTo(retrievals[i].score, 5) / MoreMath.roundTo(retrievals[i].proxScore, 5))
          + " (Vector: " + MoreMath.roundTo(retrievals[i].score, 5) + " Proximity Score: "
          + MoreMath.roundTo(retrievals[i].proxScore, 5) + ")");
    }
  }
  ...
  public static void main(String[] args) {
    ...
    InvertedIndexProx index = new InvertedIndexProx(new File(dirName), docType, stem, feedback);
    // index.print();
