...
/**
 * Docment is an abstract class that provides for tokenization of a document
 * with stop-word removal and an iterator-like interface similar to
 * StringTokenizer. Also provides a method for converting a document into a
 * vector-space bag-of-words in the form of a HashMap of tokens and their
 * occurrence counts.
 *
 * @author Ray Mooney
 */
...
public abstract class Document {
  ...
  /**
   * Creates a new Document making sure that the stopwords are loaded, indexed,
   * and ready for use. Subclasses that create concrete instances MUST call
   * prepareNextToken before finishing to ensure that the first token is
  ...
  /**
   * The nextToken slot is always precomputed and stored by this method. Performs
   * stop-word removal of candidate tokens.
   */
  ...
  /**
   * Check if this token consists of all Unicode letters to eliminate other
   * bizarre tokens
   */
  ...
  /**
   * Returns the total number of tokens in the document or -1 if there are still
   * more tokens to be read and the total count is not yet available.
   */
   ...
   * document, where each token is a key whose value is the number of times it
   * occurs in the document as stored in a Weight.
   *
   * @see Weight
   */
  ...
  public HashMapProxVector hashMapProxVector() {
    if (numTokens != 0)
      return null;
    HashMapProxVector vector = new HashMapProxVector();
    // Process each token in the document and add it to the vector
    int pos = 0;
    while (hasMoreTokens()) {
      String token = nextToken();
      vector.increment(token, pos++);
    }
    return vector;
  }

  /**
   * Compute and print out (one line per term) the term-vector (bag of words) for
   * this document
   */
